...
def cmd_chat(args):
    ...
    if not _has_any_provider_configured():
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'code-through setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: code-through gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  code-through gateway")
        ...
        print()
        print("  Or install as a service: code-through gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'code-through whatsapp' to try again.")
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        if base_url or api_key:
            deactivate_provider()
        print("Endpoint saved. Use `/model` in chat or `code-through model` to set a model.")
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    try:
        ...
        print("    4. Re-run:               code-through model")
        print()
...
def cmd_update(args):
    ...
    try:
        ...
        print("  code-through model     # Select provider and model")
...
def main():
    """Main entry point for code-through CLI."""
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    code-through              Start interactive chat
    code-through chat -q "Hello"        Single query mode
    code-through -c           Resume the most recent session
    code-through -c "my project"  Resume a session by name (latest in lineage)
    code-through --resume <session_id>  Resume a specific session by ID
    code-through setup        Run setup wizard
    code-through logout       Clear stored authentication
    code-through model        Select default model
    code-through config       View configuration
    code-through config edit  Edit config in $EDITOR
    code-through config set model gpt-4 Set a config value
    code-through gateway      Run messaging gateway
    code-through -w           Start in isolated git worktree
    code-through gateway install  Install as system service
    code-through sessions list    List past sessions
    code-through sessions browse  Interactive session picker
    code-through sessions rename ID T  Rename/title a session
    code-through update       Update to latest version

For more help on a command:
    code-through <command> --help
"""
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Hermes Agent with an interactive wizard. "
                    "Run a specific section: code-through setup model|terminal|gateway|tools|agent"
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            import shutil
            hermes_bin = shutil.which("code-through")
            if hermes_bin:
                os.execvp(hermes_bin, ["code-through", "--resume", selected_id])
            else:
    ...
    _build_codex_watch_parser(subparsers)
