...
HERMES_AGENT_LOGO = """[bold #7FDBFF]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #7FDBFF]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#5EEAD4]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#5EEAD4]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#4DA3FF]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
[#4DA3FF]╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝╚══════╝      ╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═══╝   ╚═╝[/]
[dim #5B7C99]                                                 HERMES-AGENT-Ex[/]"""
...
COMPACT_BANNER = """
[bold #4DA3FF]╔══════════════════════════════════════════════════════════════╗[/]
[bold #4DA3FF]║[/]  [#7FDBFF]⚕ HERMES-AGENT-Ex[/] [dim #5B7C99]- AI Agent Framework[/]       [bold #4DA3FF]║[/]
[bold #4DA3FF]║[/]  [#5EEAD4]High-contrast terminal UX[/]   [dim #5B7C99]Nous Research[/]    [bold #4DA3FF]║[/]
[bold #4DA3FF]╚══════════════════════════════════════════════════════════════╝[/]
"""
...
def _build_compact_banner() -> str:
    ...
    if w < 30:
        return "\n[#7FDBFF]⚕ HERMES-AGENT-Ex[/] [dim #5B7C99]- Nous Research[/]\n"
    ...
    return (
        f"\n[bold #4DA3FF]╔{bar}╗[/]\n"
        f"[bold #4DA3FF]║[/] [#7FDBFF]{line1}[/] [bold #4DA3FF]║[/]\n"
        f"[bold #4DA3FF]║[/] [dim #5B7C99]{line2}[/] [bold #4DA3FF]║[/]\n"
        f"[bold #4DA3FF]╚{bar}╝[/]\n"
    )
...
def build_welcome_banner(console: Console, model: str, cwd: str, tools: List[dict] = None, enabled_toolsets: List[str] = None, session_id: str = None, context_length: int = None):
    ...
    try:
        ...
        _accent = _bskin.get_color("banner_accent", "#5EEAD4")
        _dim = _bskin.get_color("banner_dim", "#5B7C99")
        _text = _bskin.get_color("banner_text", "#E6F1FF")
        _session_c = _bskin.get_color("session_border", "#4B5D73")
        _title_c = _bskin.get_color("banner_title", "#7FDBFF")
        _border_c = _bskin.get_color("banner_border", "#4DA3FF")
        _agent_name = _bskin.get_branding("agent_name", "HERMES-AGENT-Ex")
    except Exception:
        _bskin = None
        _accent, _dim, _text = "#5EEAD4", "#5B7C99", "#E6F1FF"
        _session_c, _title_c, _border_c = "#4B5D73", "#7FDBFF", "#4DA3FF"
...
class HermesCLI:
    ...
    def run(self):
        ...
        self._tui_style_base = {
            ...
            'hint': '#5B7C99 italic',
            # Cyan/blue horizontal rules around the input area
            'input-rule': '#4DA3FF',
