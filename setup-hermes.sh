...
# ============================================================================
# Developer bootstrap for contributors who cloned the repo manually.
# Uses uv for fast Python provisioning and editable installs.
#
# Usage:
#   ./setup-hermes.sh
#
# For normal end-user installs, prefer:
#   pipx install hermes-agent-ex
#
# This script:
...
# 5. Symlinks the 'hermes-agent-ex' CLI command into ~/.local/bin
...
echo -e "${CYAN}→${NC} Setting up hermes-agent-ex command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/hermes-agent-ex"
mkdir -p "$HOME/.local/bin"
ln -sf "$HERMES_BIN" "$HOME/.local/bin/hermes-agent-ex"
echo -e "${GREEN}✓${NC} Symlinked hermes-agent-ex → ~/.local/bin/hermes-agent-ex"
...
echo "     hermes-agent-ex-agent-ex setup"
echo ""
echo "  3. Start chatting:"
echo "     hermes-agent-ex"
echo ""
echo "Other commands:"
echo "  hermes-agent-ex status        # Check configuration"
echo "  hermes-agent-ex gateway install # Install gateway service (messaging + cron)"
echo "  hermes-agent-ex cron list     # View scheduled jobs"
echo "  hermes-agent-ex doctor        # Diagnose issues"
echo ""
