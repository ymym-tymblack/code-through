...
EXPECTED_COMMANDS = {
    ...
    "/reload-mcp", "/rollback", "/background", "/sync", "/review", "/explain", "/flow", "/diff", "/commit",
...
class TestSlashCommandCompleter:
    ...
    def test_command_option_completion_suggests_sync_actions(self):
        completions = _completions(SlashCommandCompleter(), "/sync ")

        texts = {item.text for item in completions}
        assert {"on", "off", "status", "now", "last"}.issubset(texts)
        assert any(item.display_meta_text == "Show sync status for all panes" for item in completions)
